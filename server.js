...
app.post('/api/upload', async (req, res) => {
  try {
    ...
    // Retorna o caminho relativo que funciona no site local
    res.status(200).json({ url: `../img/uploads/${safeName}` });
  } catch (error) {
    console.error('Erro no upload local:', error);
    res.status(500).json({ error: 'Erro ao salvar o arquivo localmente.' });
  }
});
